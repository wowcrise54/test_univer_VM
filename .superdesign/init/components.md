...
Framework: React 19 with custom reusable components and vanilla CSS. No third-party component library is used.

## `src/shared/ui.jsx`

Reusable interaction and layout primitives: accessible dialogs, buttons, fields, toggles, and panels. Key props are visible in each exported function signature.
...
Reusable sortable-table state, header, and row sorting helpers.

```jsx
import { useMemo, useState } from "react";

export function nextTableSort(current, key, initialDirection = "asc") {
...
export function useTableSort(defaultKey = "", defaultDirection = "asc") {
  ...
  const toggleSort = (key, initialDirection = "asc") => {
    setSort((current) => nextTableSort(current, key, initialDirection));
  };
...
export function SortableHeader({
  ...
  const ariaSort = active
    ? sort.direction === "asc"
      ...
      : "descending"
    : "none";
  const nextDirection = nextTableSort(sort, column, initialDirection).direction;
  const headerLabel = typeof children === "string" ? children : column;
  const sortActionLabel = `Сортировать «${headerLabel}» по ${nextDirection === "desc" ? "убыванию" : "возрастанию"}`;
  return (
    <th aria-sort={ariaSort} {...props}>
      <button
        aria-label={sortActionLabel}
        className={`sortable-header${active ? " is-active" : ""}`}
...
Shared formatting and option-filter helpers used by UI features.
