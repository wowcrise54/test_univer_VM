...
Полная карточка актива загружается потоковым конвейером с сохранением всех коллекций (`full=true`, лимиты и глубина не уменьшаются). По умолчанию `MPVM_ASSET_CARD_CONCURRENCY_MODE=fixed`: общий dispatcher сразу использует до `MPVM_ASSET_CARD_REQUEST_WORKERS=64` запросов в пределах `MPVM_BACKGROUND_REQUEST_LIMIT=64`; metadata, узлы, коллекции и страницы уязвимостей разделяют один пул. Значения до 128 разрешены. Для защитного режима можно выбрать `adaptive`, после чего применяются `MPVM_ASSET_CARD_INITIAL_WORKERS` и `MPVM_ASSET_CARD_MIN_WORKERS`. Статистика карточки содержит p50/p95, requests/s, критический путь и время простоя dispatcher.

Фоновые сборки карточек, постобработка сканов и загрузка деталей паспортов запускаются общим in-process `JobRunner`. PostgreSQL остаётся источником статусов: runner не допускает двойной запуск, ждёт MP VM-сессию, обновляет lease/heartbeat и возобновляет незавершённые задания после рестарта или нового подключения.

Сохранённая карточка открывается прогрессивно: сначала компактный `overview`, затем дерево конфигурации, страницы коллекций и группы уязвимостей. Полный снимок остаётся в БД и доступен через legacy endpoint; UI не загружает десятки тысяч строк одним JSON.
...
SLA компактной сводки (p95 не более одной секунды):

```powershell
python scripts/benchmark_asset_card_overview.py http://127.0.0.1:8000/api/asset-cards/<asset-id>/overview
```
...
- `GET /api/asset-cards/{assetId}/overview` - получить компактную сводку и версию снимка.
- `GET /api/asset-cards/{assetId}/configuration/tree` - получить страницу непосредственных дочерних элементов дерева.
- `GET /api/asset-cards/{assetId}/configuration/detail` - получить свойства или страницу выбранной коллекции.
- `GET /api/asset-cards/{assetId}/vulnerabilities/groups` - получить группы уязвимостей без тяжёлого списка findings.
- `GET /api/asset-cards/{assetId}/vulnerabilities/findings` - получить страницу findings выбранной группы.
