...
Актуальность карточек активов определяется по `last_seen`. Порог задаётся `MPVM_COVERAGE_STALE_DAYS` (по умолчанию 14 дней) и совпадает с индикацией свежести во вкладке «Карточки активов». Обновления и повторы запускаются через существующий центр операций.
