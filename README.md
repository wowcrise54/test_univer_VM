...
Дашборд отражает только последнее сохранённое состояние карточек и не является историческим трендом. Если хотя бы одна исходная группа в карточках была усечена, API возвращает `coverage.complete=false`: агрегаты в таком ответе считаются неполными и могут быть только нижней оценкой фактического количества.
