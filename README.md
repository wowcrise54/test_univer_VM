...
## Единый контур VM Management

Страница `/vm` объединяет запуск сканирования, постобработку, пересчёт риска, устранение и проверочное сканирование. Каждый запуск сохраняется в `vm_workflow_runs`, а его этапы — в `vm_workflow_steps`; состояние восстанавливается после перезапуска приложения. Старые страницы и API остаются доступными.

- `GET /api/vm/overview` — операционная сводка, покрытие, риск и очередь внимания;
- `GET /api/vm/workflows`, `GET /api/vm/workflows/{id}` — история и этапы выполнения;
- `POST /api/vm/workflows/scan` — управляемый scan-конвейер;
- `POST /api/vm/workflows/{id}/cancel`, `POST /api/vm/workflows/{id}/retry` — отмена и безопасный повтор;
- `POST /api/remediation/campaigns/{id}/verify` — refresh-сканирование уникальных активов кампании и автоматическая сверка кейсов.

Принятие риска и ложное срабатывание являются срочными исключениями: для них обязательны обоснование и будущий срок окончания. После истечения исключение автоматически возвращается в открытую очередь и фиксируется в аудите.
...
- `POST /api/remediation/cases/start` — идемпотентно взять находку на выбранном хосте в устранение;
...
- `GET /api/remediation/resolution-stats` — подтверждённые сканированием устранения, динамика и последние события за 7/30/90 дней.

Статистика устранений доступна отдельным разделом на странице `/vulnerabilities`. Она строится по событиям `finding_absent`, поэтому подтверждённое устранение остаётся в истории, даже если более позднее сканирование снова обнаружило уязвимость.
...
Раздел `/automations` позволяет создавать последовательные runbook-сценарии, публиковать неизменяемые версии, запускать dry-run и рабочее выполнение, а также задавать cron-расписания с IANA timezone. Встроенный шаблон «Обновление устаревших карточек» ставит на обновление карточки старше `MPVM_COVERAGE_STALE_DAYS` (по умолчанию 14 дней) ограниченными параллельными пачками до 10 карточек; после клонирования и публикации ему можно назначить ежедневное cron-расписание. Для обновления используется одна выбранная существующая задача MP VM: её scope, scanner profile, коллекторы и учётные данные сохраняются, а targets заменяются на IP текущей карточки. Если задача ещё не зарегистрирована локально, backend импортирует её конфигурацию из списка задач MP VM до запуска пачки. Планировщик не запускает один runbook параллельно и не догоняет пропущенные окна.
...
Полная карточка актива загружается параллельным конвейером с сохранением всех коллекций (`full=true`, лимиты и глубина не уменьшаются). Одновременно выполняется до `MPVM_ASSET_CARD_REQUEST_WORKERS=20` запросов, ограниченных общим потолком `MPVM_BACKGROUND_REQUEST_LIMIT=20`. Массовое обновление запускает до `MPVM_ASSET_CARD_REFRESH_WORKERS=10` независимых повторных сканирований, а `MPVM_SCAN_POSTPROCESS_WORKERS=10` позволяет одновременно отслеживать и обрабатывать их результаты. API и интерфейс могут уменьшить параллелизм отдельного запуска, но не превысить серверный лимит. Итоговая статистика карточки содержит длительности дерева, уязвимостей и сохранения, пиковый параллелизм и время ожидания очереди.

Уязвимости Docker-контейнеров собираются отдельным grouped PDQL по цепочке `Host.Softs<DockerEngine>.Containers` с join на `ImageSet.Images.Packages.@Vulners`. Нужный хост передаётся в `additionalFilterParameters.assetIds`: сам запрос не модифицируется. После получения `pdqlToken` backend читает заголовки контейнерных групп через `/api/assets_temporal_readmodel/v1/assets_grid/data`, а детальные строки пакетов и уязвимостей — через `/api/assets_temporal_readmodel/v1/assets_grid/group/data?limit=1001&pdqlToken=<token>`. Группы сохраняются по устойчивому `ContainerId`; `ImageId`, Docker Engine, пакет, `Q.VulnerStatus` и `Q.HowToFix` остаются в JSON находки. Значение `Q.Vulner.internalId` используется для связи с локальным паспортом уязвимости.

Для каждого запуска scanner task backend создаёт одну уникальную динамическую группу с predicate `(ImageSet)` после validation/precheck, дожидается её готовности и только затем запускает удалённое сканирование. Эта run-scoped группа остаётся доступной во время всего сканирования, повторных Docker PDQL-запросов и сборки карточек всех найденных активов. Каждый Docker PDQL-запрос содержит `${ASSET_SELECTOR}`, который перед отправкой заменяется на `Host.@Id = <UUID>` обрабатываемого хоста, поэтому контейнеры и уязвимости не смешиваются между активами. После terminal-состояния MP VM ей назначается сохраняемый deadline; по умолчанию группа удаляется не раньше чем через 600 секунд, а исчезновение подтверждается через `/api/assets_temporal_readmodel/v2/groups/hierarchy`. Незавершённая очистка восстанавливается после перезапуска приложения и повторяется при временной ошибке MP VM. Количество PDQL-попыток, задержка между ними, таймаут создания/удаления, период опроса и срок удержания задаются через `MPVM_DOCKER_DYNAMIC_GROUP_ITERATIONS`, `MPVM_DOCKER_DYNAMIC_GROUP_SETTLE_SECONDS`, `MPVM_DOCKER_DYNAMIC_GROUP_TIMEOUT_SECONDS`, `MPVM_DOCKER_DYNAMIC_GROUP_POLL_SECONDS` и `MPVM_DOCKER_DYNAMIC_GROUP_RETENTION_SECONDS`. Значение итераций `0` полностью отключает run-scoped группу и подготовительные Docker-запросы.
...
- `POST /api/reports/vulnerabilities/{type}/csv` - скачать детальный CSV по локальным уязвимостям ОС (`os`), ПО (`software`) или Docker-контейнеров (`docker`); пустой `asset_ids` включает все сохранённые хосты.
...
- `POST /api/asset-cards/refresh-scan/bulk` - поставить все сохранённые или только устаревшие карточки на ограниченное параллельное обновление через повторное сканирование; параметр `parallelism` задаёт число одновременных обновлений, прогресс публикуется в `/api/operations`.
- `GET /api/asset-cards/refresh-scan/templates` - получить пригодные локальные и удалённые задачи MP VM для выбора шаблона массового обновления.
...
- `admin` — все разрешения, включая пользователей, роли, диагностику и подключение MP VM;
- `operator` — чтение плюс управление задачами, операциями и карточками активов; автоматизации, SLA и безопасность исключены;
- `viewer` — чтение и формирование доступных отчётов.

Системные роли неизменяемы. Администратор может клонировать их, собрать пользовательскую роль из каталога разрешений и назначить одному пользователю несколько ролей; эффективные права объединяются. Управление пользователями, ролями и глобальными настройками доступно сразу при наличии соответствующего разрешения. Решения доступа и критические изменения сохраняются в журнале аудита на 365 дней.
