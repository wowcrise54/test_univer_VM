...
Когда postprocess обнаруживает новый актив после сканирования, перед сохранением карточки он выполняет до трёх циклов актуализации Docker-данных в MP VM. На каждой итерации создаётся уникальная временная динамическая группа в корневой группе с predicate `(@ImageSet)`, ожидается завершение операции создания и пересчёт данных, затем выполняется пробный контейнерный PDQL. В блоке `finally` группа удаляется, а её исчезновение подтверждается через `/api/assets_temporal_readmodel/v2/groups/hierarchy`; только после этого разрешена следующая итерация. Цикл завершается раньше, если контейнерные уязвимости уже доступны. Количество итераций, задержка пересчёта, таймаут и период опроса задаются через `MPVM_DOCKER_DYNAMIC_GROUP_ITERATIONS`, `MPVM_DOCKER_DYNAMIC_GROUP_SETTLE_SECONDS`, `MPVM_DOCKER_DYNAMIC_GROUP_TIMEOUT_SECONDS` и `MPVM_DOCKER_DYNAMIC_GROUP_POLL_SECONDS`. Значение итераций `0` отключает подготовительный цикл.
