...
Страница `/coverage` сопоставляет локальный реестр активов с карточками и показывает отсутствующие, устаревшие, усечённые и неудачно обновлённые карточки. Порог свежести задаётся `MPVM_COVERAGE_STALE_DAYS` (по умолчанию 7). Возраст считается по `last_seen` — той же дате, которая отображается как «Обновлено» во вкладке «Карточки активов». Обновления и повторы запускаются через существующий центр операций.
...
Системные роли неизменяемы. Администратор может клонировать их, собрать пользовательскую роль из каталога разрешений и назначить одному пользователю несколько ролей; эффективные права объединяются. Управление пользователями, ролями и глобальными настройками требует повторного ввода пароля и открывает 30-минутное окно подтверждения. Решения доступа и критические изменения сохраняются в журнале аудита на 365 дней.
