...
function TaskBuilderPanel({ defaults, lookups, selectedTask, selectedTaskId, setSelectedTaskId, refreshTasks, busy, runBusy, showAlert, session, systemStatus }) {
  ...
  useEffect(() => {
    ...
    try {
      const saved = JSON.parse(window.localStorage.getItem("mpvm.task-draft.v1") || "null");
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      window.localStorage.setItem("mpvm.task-draft.v1", JSON.stringify({ version: 1, saved_at: new Date().toISOString(), form }));
    }, 350);
  ...
  const preflight = [
    { label: "PostgreSQL", ok: systemStatus?.components?.database?.state === "ok", detail: "Нужен для истории операции и результатов" },
    { label: "Сессия MP VM", ok: Boolean(session?.connected), detail: "Подключитесь к MP VM" },
    { label: "Справочники", ok: Boolean(lookups.scopes.length && lookups.scanner_profiles.length), detail: "Загрузите scopes и scanner profiles" },
    { label: "Выбрана задача", ok: Boolean(selectedTaskId), detail: "Создайте или выберите задачу" },
    { label: "Scope и профиль", ok: Boolean(form.scope_id && form.profile_id), detail: "Заполните обязательные параметры" },
    { label: "Цели сканирования", ok: splitTokens(form.include_targets).length > 0, detail: "Добавьте хотя бы одну цель" },
  ];
  ...
  const actionTask = (action, key, successText, body = null) =>
    runBusy(key, async () => {
      ...
      if (action === "start") options.headers = { "X-Idempotency-Key": createIdempotencyKey("scan-start") };
  ...
  return (
    ...
    >
      ...
      <section className={`task-preflight ${preflightReady ? "task-preflight--ready" : ""}`}>
        <div><strong>Проверка перед запуском</strong><span>{preflightReady ? "Все условия выполнены" : "Запуск заблокирован до устранения замечаний"} · черновик сохраняется автоматически</span></div>
        <ul>{preflight.map((item) => <li className={item.ok ? "is-ok" : "is-error"} key={item.label}><span>{item.ok ? "✓" : "!"}</span><div><strong>{item.label}</strong><small>{item.ok ? "Готово" : item.detail}</small></div></li>)}</ul>
      </section>
      <div className="action-row">
        ...
        <Button variant="success" disabled={!preflightReady} busy={busy.startTask} onClick={() => actionTask("start", "startTask", startSuccessText, startPayload())}>Запустить</Button>
  ...
  return (
    ...
    >
      ...
      <div className="task-table-footer">
        ...
        <Button variant="danger" busy={Boolean(deletingId && deletingId === selectedTaskId)} disabled={!selectedTaskId} onClick={() => setPendingDeleteId(selectedTaskId)}>
          Удалить выбранную
        </Button>
      </div>
    ...
    <ConfirmDialog
      ...
      impact={[`Метод удаления: ${mode}`, "Запущенная задача может перестать быть доступной в MP VM", "Локальная запись удалится только после подтверждённого ответа MP VM"]}
  ...
  return (
    ...
    >
            ...
            {cards.length ? cards.map((row) => {
              ...
              return (
                <tr key={row.asset_id}>
                  ...
                  <td><FreshnessBadge value={row.last_seen} source="PostgreSQL" /></td>
                  <td>
      ...
      <ConfirmDialog
        ...
        impact={["Актив в MP VM не изменится", "Карточку можно собрать заново из MP VM"]}
        confirmLabel="Удалить из БД"
        busy={Boolean(pendingCardDelete && busy[`assetCardDelete:${pendingCardDelete.asset_id}`])}
  ...
  return (
    ...
    >
          ...
          <tbody>
            {pageRows.length ? pageRows.map((row, rowIndex) => (
              <tr key={row.internal_id || row.external_id || `${row.name}-${rowIndex}`} className={selected?.internal_id === row.internal_id ? "is-selected" : ""}>
                ...
                <td>{row.has_detail || row.raw_detail ? <FreshnessBadge value={row.detail_updated_at || row.last_seen} source="PostgreSQL" /> : "нет"}</td>
                ...
                <td>
                  <div className="row-actions">
                    ...
                    <Button variant="tiny-danger" busy={busy[`passportDelete:${row.internal_id}`]} onClick={() => setPendingPassportDelete(row)}>Удалить</Button>
                  </div>
                </td>
              </tr>
            )) : (
      ...
      <ConfirmDialog
        ...
        impact={["Паспорт в MP VM не изменится", "Данные можно повторно загрузить через PDQL"]}
        confirmLabel="Удалить из БД"
        busy={Boolean(pendingPassportDelete && busy[`passportDelete:${pendingPassportDelete.internal_id}`])}
          ...
          <tbody>
            {rows.length ? rows.map((row) => (
              <tr key={row.id}>
                ...
                <td>{row.cve}</td>
                <td><Severity value={row.severity} /></td>
                <td><FreshnessBadge value={row.created_at} source="PostgreSQL" /></td>
              </tr>
            )) : (
              <tr><td colSpan={8} className="empty-cell">Нет данных. Импортируйте CSV или выполните PDQL export.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function FreshnessBadge({ value, source }) {
  ...
  const state = ageHours == null ? "unknown" : ageHours <= 24 ? "fresh" : ageHours <= 168 ? "aging" : "stale";
  const label = state === "fresh" ? "свежие" : state === "aging" ? "требуют обновления" : state === "stale" ? "устарели" : "время неизвестно";
  return <span className={`freshness freshness--${state}`} title={value ? formatDateTime(value) : "Время обновления отсутствует"}><strong>{label}</strong><small>{source}</small></span>;
}
